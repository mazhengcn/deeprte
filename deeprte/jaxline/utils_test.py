...
class TestBroadcast(absltest.TestCase):
    def test_bcast_local_devices(self):
        self.assertEqual(
            utils.bcast_local_devices(jnp.zeros([])),
            jnp.zeros([jax.local_device_count()]),
        )

        self.assertEqual(
            utils.bcast_local_devices(jnp.ones([])),
...
class TestSpecializeRngHostDevice(absltest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.rng = jnp.broadcast_to(rng, (jax.local_device_count(),) + rng.shape)
    ...
    def test_unique_device(self):
        ...
        specialize_func = jax.pmap(
            functools.partial(
                utils.specialize_rng_host_device, axis_name="i", mode=mode
            ),
            axis_name="i",
        )
    ...
    def test_same_device(self):
        ...
        self.assertEqual(np.unique(rng, axis=0).shape[0], 1)
