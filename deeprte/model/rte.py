...
from deeprte.model import integrate, modules
...
class RTEOp(Solution):
    def forward(
        self, r, v, sigma: modules.FunctionInputs, psi_bc: modules.FunctionInputs
    ) -> jnp.ndarray:
        green_func = modules.GreenFunctionNet(self.config.green_function)
        psi_func = integrate.quad(
            green_func, [psi_bc.x, psi_bc.f], argnum=1, use_hk=True
        )

        rv = jnp.concatenate([r, v])
        psi = psi_func(rv, sigma)
