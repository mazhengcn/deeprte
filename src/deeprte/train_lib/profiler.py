...
class Profiler:
    ...
    def __init__(self, config, offset_step=0):
        ...
        self.start_initial_profile_step = self._set_first_profiler_step(
            config.skip_first_n_steps_for_profiler, offset_step
        )
        self.finished_initial_profile_step = self._set_last_profiler_step(
            config.profiler_steps, config.steps
        )
    ...
    def maybe_activate_profiler(self, step, state):
        ...
        """
        if self.mode != "" and (
            step == self.start_initial_profile_step
            or self.should_activate_periodic_profile(step)
        ):
    ...
    def maybe_deactivate_profiler(self, step, state):
        ...
        if self.mode != "" and (
            ...
            or self.should_deactivate_periodic_profile(step)
        ):
    ...
    def _set_last_profiler_step(self, profiler_steps, last_job_step):
        return min(
            self.start_initial_profile_step + profiler_steps - 1, last_job_step - 1
        )

    def should_activate_periodic_profile(self, step):
        return (
            self.profile_period > 0
            and (step - self.start_initial_profile_step) % self.profile_period == 0
        )

    def should_deactivate_periodic_profile(self, step):
        return (
            self.profile_period > 0
            and (step - self.finished_initial_profile_step) % self.profile_period == 0
        )
