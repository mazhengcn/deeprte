...
if params.generate_train_data == true
    [boundary, rand_params] = gen_train_boundary(params, rand_params);
else
    [boundary, rand_params] = gen_test_boundary(params, rand_params);
end
...
% generate scattering kernel
function [sigma, q, varepsilon, rand_params] = gen_coef(params, rand_params)
...
rand_params.in_sigma_a = in_sigma_a;
rand_params.out_sigma_a = out_sigma_a;
rand_params.in_sigma_t = in_sigma_t;
rand_params.out_sigma_t = out_sigma_t;

end

% generate boundary
function [boundary, rand_params] = gen_train_boundary(params, rand_params)
...
y_l = rand_params.r_ind(1) * hy + yl;
y_r = yr - rand_params.r_ind(2) * hy;
x_b = xr - rand_params.r_ind(3) * hx;
x_t = rand_params.r_ind(4) * hx + xl;

func_psiL = @(x, y)(exp(- (y - y_l) .^ 2/2 / rand_params.variance_x(1)) * exp(- (x - xl) .^ 2/2 / rand_params.variance_x(1)));
func_psiL_v = @(x)(exp(- (x - x(rand_params.v_ind(1))) .^ 2/2 / rand_params.variance_v(1)));
func_psiR = @(x, y)(exp(- (y - y_r) .^ 2/2 / rand_params.variance_x(2)) * exp(- (x - xr) .^ 2/2 / rand_params.variance_x(2)));
func_psiR_v = @(x)(exp(- (x - x(rand_params.v_ind(2))) .^ 2/2 / rand_params.variance_v(2)));
func_psiB = @(x, y)(exp(- (y - yl) .^ 2/2 / rand_params.variance_x(3)) * exp(- (x - x_b) .^ 2/2 / rand_params.variance_x(3)));
func_psiB_v = @(x)(exp(- (x - x(rand_params.v_ind(3))) .^ 2/2 / rand_params.variance_v(3)));
func_psiT = @(x, y)(exp(- (y - yr) .^ 2/2 / rand_params.variance_x(4)) * exp(- (x - x_t) .^ 2/2 / rand_params.variance_x(4)));
func_psiT_v = @(x)(exp(- (x - x(rand_params.v_ind(4))) .^ 2/2 / rand_params.variance_v(4)));
...
func_psiL = @(x, y)(1/sum_x_L * exp(- (y - y_l) .^ 2/2 / rand_params.variance_x(1)) * exp(- (x - xl) .^ 2/2 / rand_params.variance_x(1)));
func_psiL_v = @(x)(1/sum_v_L *exp(- (x - x(rand_params.v_ind(1))) .^ 2/2 / rand_params.variance_v(1)));
% func_psiL_v = @(x)(exp(- (x - x(v_index(1))).^2/2 / variance_v(1)));
func_psiR = @(x, y)(1/sum_x_R * exp(- (y - y_r) .^ 2/2 / rand_params.variance_x(2)) * exp(- (x - xr) .^ 2/2 / rand_params.variance_x(2)));
func_psiR_v = @(x)(1/sum_v_R *exp(- (x - x(rand_params.v_ind(2))) .^ 2/2 / rand_params.variance_v(2)));
func_psiB = @(x, y)(1/sum_x_B * exp(- (y - yl) .^ 2/2 / rand_params.variance_x(3)) * exp(- (x - x_b) .^ 2/2 / rand_params.variance_x(3)));
func_psiB_v = @(x)(1/sum_v_B *exp(- (x - x(rand_params.v_ind(3))) .^ 2/2 / rand_params.variance_v(3)));
func_psiT = @(x, y)(1/sum_x_T * exp(- (y - yr) .^ 2/2 / rand_params.variance_x(4)) * exp(- (x - x_t) .^ 2/2 / rand_params.variance_x(4)));
func_psiT_v = @(x)(1/sum_v_T *exp(- (x - x(rand_params.v_ind(4))) .^ 2/2 / rand_params.variance_v(4)));
...
% generate sigma
function [scattering_kernel, rand_params] = gen_scattering_kernel(g_scope, N, rand_params)

[start, range] = get_rand_params(g_scope);
rand_params.g = rand()*(range)+start;
scattering_kernel = P2generator(N, rand_params.g);
